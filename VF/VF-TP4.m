...
#time discretization
global tf = 1.5; # final simulation time
global dtinit = 0.02; # initial time step
global dtmax = 0.02; # maximum time step 
global Ndtmax = 10000; # maximum number of time step


#Newton convergence 
global Newtmax = 30; # maximum number of Newton iterations 
global eps=1.0E-6; #  stopping criteria 


global Km = 1;
global Kc = 10;  # Kc/Km ratio of diffusion coefficients between continental and marine environments 
...
for i=1:Nint
  k1 = cellint(i,1);
  k2 = cellint(i,2);    
  fluxint(i) = Tint(i)*(f_psi(b(k2))-fpsi(b(k1)));
  if (fluxint(i)>0) then 
      upwind(i) = k1 
  else 
      upwind(i) = k2
...
for i=1:Nint
  ...
  k2 = cellint(i,2);
  kup = upwind(i);
  flux_lito1 = Ki(1)*cs(kup)*fluxint(i)
  flux_lito2 = Ki(2)*(1-cs(kup))*fluxint(i)
  R(k1,1) = R(k1,1) + flux_lito1
  R(k1,2) = R(k1,2) + flux_lito2
  R(k2,1) = R(k2,1) - flux_lito1
  R(k2,2) = R(k2,2) - flux_lito2
end

# input boundary fluxes
ib = cellbound(1)
R(ib,1) = R(ib,1) + g0(1)
R(ib,2) = R(ib,2) + g0(2)
...
for i=1:Nint
  ...
  flux1_ckup = Ki(1)*fluxint(i)
  flux1_hk1 =  Ki(1)*cs(kup)*(f_psip(b(k1)))
  flux1_hk2 =  Ki(1)*cs(kup)*(-f_psip(b(k2)))

  flux2_ckup = -Ki(2)*fluxint(i)
  flux2_hk1 =  Ki(2)*(1-cs(kup))*(f_psip(b(k1)))
  flux2_hk2 =  Ki(2)*(1-cs(kup))*(-f_psip(b(k2)))

  A(2*k1-1,2*kup) = A(2*k1-1,2*kup) + flux1_ckup
  A(2*k1-1,2*k1-1) = A(2*k1-1,2*k1-1) + flux1_hk1
  A(2*k1-1,2*k2-1) = A(2*k1-1,2*k2-1) + flux1_hk2
  
  A(2*k1,2*kup) = A(2*k1,2*kup)+flux2_ckup
  A(2*k1,2*k1-1) = A(2*k1,2*k1-1)+flux2_hk1
  A(2*k1,2*k2-1) = A(2*k1,2*k2-1)+flux2_hk2
  
  A(2*k2-1,2*kup) = A(2*k1-1,2*kup) - flux1_ckup
  A(2*k2-1,2*k1-1) = A(2*k1-1,2*k1-1) - flux1_hk1
  A(2*k2-1,2*k2-1) = A(2*k1-1,2*k2-1) - flux1_hk2
  
  A(2*k2,2*kup) = A(2*k1,2*kup)-flux2_ckup
  A(2*k2,2*k1-1) = A(2*k1,2*k1-1)-flux2_hk1
  A(2*k2,2*k2-1) = A(2*k1,2*k2-1)-flux2_hk2
