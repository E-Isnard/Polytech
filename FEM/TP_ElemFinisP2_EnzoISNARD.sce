clear
function fb = TheRhs(mu,x,y)    
    fb   = mu*5*%pi^2*sin(1.0*%pi.*x).*sin(2.0*%pi.*y);
endfunction
...
// Construction de la numérotation du maillage
...
for ib = 1:Nddl_bord;
   ...
   errorL2_vec(msize) = sqrt(sum((Var_P2 -Var_Exact).^2 ))
   errorLinf_vec(msize) = max(abs(Var_P2 - Var_Exact))

end

// Régression linéaire entre log(h_vec) et log(ErrorsS_L2) pour déterminer l'ordre de convergence en norme L2
[a,b,sig] = reglin(log(h_vec),log(errorL2_vec)')
disp("L ordre de convergence en norme L2 en P2 est "+string(a))
// Régression linéaire entre log(h_vec) et log(ErrorsS_Li) pour déterminer l'ordre de convergence en norme L_inf
[a,b,sig] = reglin(log(h_vec),log(errorLinf_vec)')
disp("L ordre de convergence en norme L_inf en P2 est "+string(a))


loglog(h_vec',[errorL2_vec,errorLinf_vec],"-o")
title("Graphe des erreur L2 et L_inf")
xgrid()
xlabel("h")
ylabel("Erreur")
legend(["Erreur L2"],["Erreur Linf"])
