...
public class DayOK {
    ...
    /**
     * Constructor for objects of class DayOK.
     * 
     * @param dayNumber The number of this day in the year (1-366).
     */
    public DayOK(int dayNumber) {
     ...
     *         -1 if there is insufficient space.
     */
        ...
        for (int slot = 0; slot < MAX_APPOINTMENTS_PER_DAY; slot++) {
            if (appointments[slot] == null) {
                ...
                if (duration == 1) {
                    ...
                    for (int nextSlot = slot + 1; further_slots_required > 0
                            && appointments[nextSlot] == null; nextSlot++) {
    ...
    public boolean makeAppointment(int time, Appointment appointment) {
        if (findSpace(appointment) != -1) {
            if (time == 13 && appointment.getDuration()==3) {
                return false;
            }
    ...
    /**
     * @param Which time of day. This must be between the START_OF_DAY time and the
